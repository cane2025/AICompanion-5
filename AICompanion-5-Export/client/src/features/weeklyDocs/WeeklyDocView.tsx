          ...
          <div className="space-y-1">
            {Object.entries(categories).map(([category, hours]) => (
              <div key={category} className="flex justify-between text-sm">
                <span>{category}</span>
                <Badge variant="outline">{String(hours)}h</Badge>
              </div>
            ))}
          </div>
        </div>
...
function DayEditor({
  ...
  const addEntryMut = useMutation({
    mutationFn: (entryData: any) => createWeeklyDocEntry(doc.id, entryData),
    onSuccess: () => {
...
export default function WeeklyDocView({ clientId }: WeeklyDocViewProps) {
  ...
  const { data = [], isLoading } = useQuery({
    ...
    queryFn: () => listWeeklyDocs(clientId),
  });
