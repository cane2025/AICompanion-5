...
export class EmailService {
  ...
  private getEmailTemplate(notification: EmailNotification): {
    html: string;
    text: string;
  } {
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f8fafc; padding: 15px; border-radius: 8px; margin: 20px 0;">
          ...
          <ul>
            ...
            <li><strong>Uppdaterad:</strong> ${format(
...
- Status: ${data.status}
- Uppdaterad: ${format(new Date(data.updatedAt), "dd MMMM yyyy HH:mm", {
      locale: sv,
    })}
...
Detta är ett automatiskt meddelande från vårdplaneringssystemet.
  ...
  private getMonthlyReportDueTemplate(data: any): {
    html: string;
    text: string;
  } {
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #fef2f2; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #dc2626;">
          ...
          <ul>
            <li><strong>Klient:</strong> ${data.clientInitials}</li>
            <li><strong>Period:</strong> ${format(
              new Date(data.year, data.month - 1),
              "MMMM yyyy",
              { locale: sv }
            )}</li>
            <li><strong>Förfallodatum:</strong> ${format(
...
Detaljer:
- Klient: ${data.clientInitials}
- Period: ${format(new Date(data.year, data.month - 1), "MMMM yyyy", {
      locale: sv,
    })}
- Förfallodatum: ${format(new Date(data.dueDate), "dd MMMM yyyy", {
      locale: sv,
    })}
- Status: ${data.status}
...
Logga in i systemet för att slutföra rapporten.
  ...
  private getWeeklyDocReminderTemplate(data: any): {
    html: string;
    text: string;
  } {
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #fff7ed; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #ea580c;">
          ...
          <ul>
            ...
            <li><strong>Ofullständiga dagar:</strong> ${data.incompleteDays.join(
              ", "
            )}</li>
          </ul>
        </div>
  ...
  private getSystemAlertTemplate(data: any): { html: string; text: string } {
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #fef2f2; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #dc2626;">
          <h3>Detaljer:</h3>
          <p><strong>Tidpunkt:</strong> ${format(
            new Date(),
            "dd MMMM yyyy HH:mm",
            { locale: sv }
          )}</p>
          ${
            data.details
              ? `<p><strong>Ytterligare information:</strong> ${data.details}</p>`
              : ""
          }
        </div>
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f0fdf4; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #059669;">
          ...
          <ul>
            ...
            <li><strong>Tidpunkt:</strong> ${format(
              new Date(),
              "dd MMMM yyyy HH:mm",
              { locale: sv }
            )}</li>
        ...
        ${
          data.summary
            ? `<p><strong>Sammanfattning:</strong> ${data.summary}</p>`
            : ""
        }
...
Detta är ett automatiskt meddelande från vårdplaneringssystemet.
  ...
  async sendBulkNotifications(
    ...
    if (failed.length > 0) {
      ...
      throw new Error(
        `${failed.length} e-postmeddelanden misslyckades att skickas`
      );
    }
  }
