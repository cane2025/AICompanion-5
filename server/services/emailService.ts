...
const emailTemplates = {
  ...
  dailySummary: (staffName: string, stats: any) => ({
    subject: `Daglig sammanfattning - ${format(new Date(), "d MMMM yyyy", {
      locale: sv,
    })}`,
    html: `
      <h2>Hej ${staffName},</h2>
...
export class EmailService {
    ...
    try {
      ...
      const info = await this.transporter.sendMail({
        from:
          ...
          '"Uppföljningssystemet" <noreply@uppfoljning.se>',
        to,
        subject,
        html,
      });
  ...
  async sendBulkEmails(
    ...
    const results = await Promise.allSettled(
      recipients.map(({ email, template, args }) =>
        this.sendEmail(email, template, ...args)
      )
    );
