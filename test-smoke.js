...
async function main() {
  ...
  await test("Create care plan", async () => {
    ...
    if (!newCarePlan.id)
      throw new Error("Care plan creation failed - no ID returned");
  });
  ...
  await test("Create implementation plan", async () => {
    ...
    if (!newImplPlan.id)
      throw new Error("Implementation plan creation failed - no ID returned");
  });
  ...
  await test("Create weekly documentation", async () => {
    ...
    if (!newWeeklyDoc.id)
      throw new Error("Weekly documentation creation failed - no ID returned");
  });
  ...
  await test("Create monthly report", async () => {
    ...
    if (!newMonthlyReport.id)
      throw new Error("Monthly report creation failed - no ID returned");
  });
  ...
  await test("Create Vimsa time", async () => {
    ...
    if (!newVimsaTime.id)
      throw new Error("Vimsa time creation failed - no ID returned");
  });
  ...
  await test("Update implementation plan", async () => {
    const updated = await request(
      "PUT",
      `/implementation-plans/${newImplPlan.id}`,
      {
  ...
  await test("Update monthly report", async () => {
    const updated = await request(
      "PUT",
      `/monthly-reports/${newMonthlyReport.id}`,
      {
        approved: false,
  ...
  await test("Get clients by staff", async () => {
    ...
    if (!Array.isArray(clients))
      throw new Error("Clients by staff not an array");
  ...
  await test("Get care plans by staff", async () => {
    const carePlans = await request(
      "GET",
      `/care-plans/staff/${staffList[0].id}`
    );
    if (!Array.isArray(carePlans))
      throw new Error("Care plans by staff not an array");
  });

  await test("Get implementation plans by client", async () => {
    const implPlans = await request(
      "GET",
      `/implementation-plans/${newClient.id}`
    );
    if (!Array.isArray(implPlans))
      throw new Error("Implementation plans by client not an array");
  });
