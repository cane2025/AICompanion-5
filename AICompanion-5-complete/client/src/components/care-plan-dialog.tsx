...
interface CarePlanDialogProps {
  ...
  selectedStaffId?: string; // Add prop for selected staff from left panel
}
...
export function CarePlanDialog({
  ...
  const form = useForm<CarePlanFormData>({
    ...
    defaultValues: {
      staffId: selectedStaffId || "", // Default to selected staff from left panel
      clientInitials: "",
      planNumber: "",
      receivedDate: new Date().toISOString().split("T")[0],
      assignedStaffIds: selectedStaffId ? [selectedStaffId] : [], // Auto-assign selected staff
      journalDate: "",
      comment: "",
    },
  });
  ...
  React.useEffect(() => {
    if (selectedStaffId) {
      ...
      form.setValue("assignedStaffIds", [selectedStaffId]);
    }
          ...
          <Form {...form}>
            ...
            >
              <Controller
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Ansvarig behandlare *</FormLabel>
                    <Select value={field.value} onValueChange={field.onChange}>
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue placeholder="Välj ansvarig behandlare" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                        {staff?.map((s) => (
