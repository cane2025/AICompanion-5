...
export const getClients = (): Promise<Client[]> =>
  fetch(`${API_BASE_URL}/clients`, { credentials: "include" }).then((res) =>
...
// Example of using buildPayload in an API call
...
// Generic API helper for dev usage with token header
...
export async function apiRequest(path: string, opts: RequestInit = {}) {
  const token =
    typeof window !== "undefined" ? localStorage.getItem("devToken") : null;
  const headers = {
    "Content-Type": "application/json",
